...
public class AOTVWaypointsPage extends Page {
    ...
    public AOTVWaypointsPage() {
        ...
        addNewList.setClickAction(() -> {
            if (BetterMining.aotvWaypoints == null || BetterMining.aotvWaypoints.getRoutes() == null) return;

            BetterMining.aotvWaypoints.getRoutes().add(new AOTVWaypointsStructs.WaypointList("New Route", false, false, new ArrayList<>()));
            redrawRoutes();
        });
        importList.setClickAction(() -> {
            try {
                ...
                if (data.startsWith("#betterminingWaypoint#::")) {
                    data = data.replace("#betterminingWaypoint#::", "");
                    data = new String(Base64.getDecoder().decode(data));
                    AOTVWaypointsStructs.WaypointList list = BetterMining.gson.fromJson(data, AOTVWaypointsStructs.WaypointList.class);
                    if (list != null) {
                        list.enabled = false;
                        BetterMining.aotvWaypoints.getRoutes().add(list);
                        Notifications.INSTANCE.send("BetterMining", "Imported route " + list.name + " from BetterMining successfully!");
                        redrawRoutes();
                    }
                    ...
                    data = sb.toString();
                    CategoryList categoryList = BetterMining.gson.fromJson(data, CategoryList.class);
                    ...
                    for (WaypointCategory category : categories) {
                        ...
                        BetterMining.aotvWaypoints.getRoutes().add(list);
                        Notifications.INSTANCE.send("BetterMining", "Imported route " + list.name + " from Skytils successfully!");
                    }
                    ...
                    data = new String(Base64.getDecoder().decode(data));
                    JsonElement element = BetterMining.gson.fromJson(data, JsonElement.class);
                    if (element instanceof JsonObject) {
                        ...
                        if (object.has("routes")) {
                            ...
                            Notifications.INSTANCE.send("BetterMining", "Imported routes from BetterMining successfully!");
                            redrawRoutes();
                        }
                    } else if (element instanceof JsonArray) {
                        JsonArray array = (JsonArray) element;
                        ArrayList<Waypoint> waypoints = BetterMining.gson.fromJson(element, new TypeToken<ArrayList<Waypoint>>() {
                        }.getType());
                ...
                Notifications.INSTANCE.send("BetterMining", "Failed to import route!");
                Notifications.INSTANCE.send("BetterMining", "Error message: " + e.getMessage());
                Notifications.INSTANCE.send("BetterMining", "Click me to copy the message.", 5000, () -> {
                    Clipboard clipboard = Toolkit.getDefaultToolkit().getSystemClipboard();
    ...
    public static void redrawRoutes() {
        ...
        if (BetterMining.aotvWaypoints == null || BetterMining.aotvWaypoints.getRoutes().size() == 0) return;
        ...
        for (AOTVWaypointsStructs.WaypointList list : BetterMining.aotvWaypoints.getRoutes()) {
            ...
            selected.setClickAction(() -> {
                BetterMining.aotvWaypoints.getRoutes().forEach(waypointList -> waypointList.enabled = false);
                BetterMining.aotvWaypoints.getRoutes().get(BetterMining.aotvWaypoints.getRoutes().indexOf(list)).enabled = true;
                redrawRoutes();
            });
            ...
            expandButton.setClickAction(() -> {
                if (Objects.equals(expandButton.getText(), "Expand")) {
                    BetterMining.aotvWaypoints.getRoutes().forEach(waypointList -> waypointList.showCoords = false);
                    BetterMining.aotvWaypoints.getRoutes().get(BetterMining.aotvWaypoints.getRoutes().indexOf(list)).showCoords = true;
                } else if (Objects.equals(expandButton.getText(), "Hide")) {
                    BetterMining.aotvWaypoints.getRoutes().get(BetterMining.aotvWaypoints.getRoutes().indexOf(list)).showCoords = false;
                }
                redrawRoutes();
            });
            ...
            BasicButton deleteButton = new BasicButton(80, BasicButton.SIZE_36, "Delete", null, null, BasicButton.ALIGNMENT_CENTER, ColorPalette.PRIMARY_DESTRUCTIVE);
            deleteButton.setClickAction(() -> {
                BetterMining.aotvWaypoints.getRoutes().remove(list);
                redrawRoutes();
            });
            ...
            shareButton.setClickAction(() -> {
                String routeString = Base64.getEncoder().encodeToString(BetterMining.gson.toJson(list).getBytes());
                StringSelection stringSelection = new StringSelection("#betterminingWaypoint#::" + routeString);
                Clipboard clipboard = Toolkit.getDefaultToolkit().getSystemClipboard();
                clipboard.setContents(stringSelection, null);
            });
    ...
    public void draw(long vg, int x, int y, InputHandler inputHandler) {
        ...
        if (BetterMining.aotvWaypoints == null || BetterMining.aotvWaypoints.getRoutes().size() == 0 || routes.size() == 0) {
