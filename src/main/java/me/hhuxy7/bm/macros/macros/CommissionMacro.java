...
import me.hhuxy7.bm.features.PingAlert;
...
import me.hhuxy7.bm.utils.HypixelUtils.*;
...
public class CommissionMacro extends Macro {
    ...
    protected void onEnable() {
        ...
        regenMana = false;
        occupiedCounter = 0;
        disableOnLimbo = BetterMining.config.stopOnLimbo;
        commissionCount = 0;
        runTime.reset();
        isWarping = true;
        ...
        if (BetterMining.config.commAutoPickonimbusSwapper) {
        ...
        if (BetterMining.config.fastMine) {
    ...
    public void onTick(TickEvent.Phase phase) {
        ...
        if (!isWarping && comissionState == State.COMMITTING) {
            ...
            if (pickonimbusState != PickonimbusState.GET && BetterMining.config.commAutoPickonimbusSwapper) return;
        }
        switch (reWarpState) {
            case WARP_SB:
        ...
        if (regenMana) {
            ...
            if (manaRegenTimer.hasReached(BetterMining.config.manaRegenTime * 1000L)) {
                regenMana = false;
            }
            return;
        }
        ...
        switch (comissionState) {
                            ...
                            case WARP_TO_EMISSARY:
                                switch (warpToEmissaryState) {
                                    ...
                                    case LOOK:
                                        if (nextActionDelay.hasReached(50)) {
                                            ...
                                            if (!rotation.completed) {
                                                rotation.initAngleLock(yawPitchGoal.getLeft(), yawPitchGoal.getRight(), BetterMining.config.commCameraWaypointSpeed + lookTimeIncrement);
                                            }
                                    ...
                                    case WARP:
                                        ...
                                        break;
                                    case ARRIVE_CHECK:
                                        if (nextActionDelay.hasReached(BetterMining.config.commArriveWaitTime)) {
            ...
            case EXIT_COMM_MENU:
                ...
                break;
            case GET_COMMISSION:
                if (nextActionDelay.hasReached(BetterMining.config.commReadOutDelay)) {
            ...
            case COMMITTING:
                if (typeOfCommission == TypeOfCommission.MINING_COMM) {
                    if (BetterMining.config.refuelWithAbiphone) {
                    ...
                    case MINING_COMM:
                        ...
                        switch (baritone.getState()) {
                            ...
                            case FAILED:
                                ...
                                if (ray != null && baritone.getCurrentBlockPos() != null && ray.getBlockPos().equals(baritone.getCurrentBlockPos()) && noDmgCount > BetterMining.config.mithRestartTimeThreshold * 20) {
    ...
    private BaritoneConfig getMineBehaviour() {
        return new BaritoneConfig(
                MiningType.STATIC,
                BetterMining.config.commShiftWhenMine,
                true,
                false,
                BetterMining.config.commRotationTime,
                BetterMining.config.commRestartTimeThreshold,
                null,
                null,
                256,
                0
