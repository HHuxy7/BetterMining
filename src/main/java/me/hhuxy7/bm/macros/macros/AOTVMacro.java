...
public class AOTVMacro extends Macro {
    ...
    public void onEnable() {
        ...
        if (BetterMining.aotvWaypoints.getSelectedRoute() == null) {
        ...
        if (BetterMining.config.aotvKillYogs) {
            BetterMining.mobKiller.toggle();
            MobKiller.setMobsNames(false, "Yog");
            if (BetterMining.config.useHyperionUnderPlayer) {
                MobKiller.scanRange = Math.min(5, BetterMining.config.mobKillerScanRange);
            } else {
                MobKiller.scanRange = BetterMining.config.mobKillerScanRange;
            }
        ...
        if (event.old.getBlock() == Blocks.cobblestone) {
            if (event.update.getBlock() == Blocks.air) {
                ...
                if (wp != null) {
                    ...
                    if (BetterMining.config.stopIfCobblestoneDestroyed) {
                        this.toggle();
    ...
    public void onTick(TickEvent.Phase phase) {
        ...
        switch (currentState) {
            ...
            case WARPING: {
                ...
                if (BetterMining.config.aotvTeleportThreshold > 0) {
                    if (!firstTp && !timeBetweenLastWaypoint.hasReached((long) (BetterMining.config.aotvTeleportThreshold * 1000))) {
                ...
                if (!rotation.rotating)
                    rotation.initAngleLock(waypoint, BetterMining.config.aotvCameraWaypointSpeed);
    ...
    private BaritoneConfig getAutoMineConfig() {
        return new BaritoneConfig(
                ...
                false,
                true,
                BetterMining.config.aotvCameraSpeed,
                BetterMining.config.aotvRestartTimeThreshold, //changed with config
                null,
                null,
                256,
                0
