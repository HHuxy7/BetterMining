package me.hhuxy7.bm.macros.macros;
...
import me.hhuxy7.bm.features.Autosell;
...
import me.hhuxy7.bm.utils.BlockUtils.BlockData;
import me.hhuxy7.bm.utils.BlockUtils.BlockUtils;
import me.hhuxy7.bm.utils.BlockUtils.Box;
import me.hhuxy7.bm.utils.BlockUtils.OffsetBox;
import me.hhuxy7.bm.utils.PlayerUtils;
import me.hhuxy7.bm.utils.Timer;
import me.hhuxy7.bm.utils.Utils.MathUtils;
import me.hhuxy7.bm.utils.Utils.ThreadUtils;
...
import static me.hhuxy7.bm.utils.AngleUtils.*;
import static me.hhuxy7.bm.utils.BlockUtils.BlockUtils.*;
...
public class PowderMacro extends Macro {
    ...
    public void onEnable() {
        ...
        if(BetterMining.config.playerFailsafe) {
            if (PlayerUtils.isNearPlayer(BetterMining.config.playerRad)) {
                LogUtils.addMessage("Not starting, there is a player nearby");
                this.toggle();
                return;
            }
        }
        if(BetterMining.config.powBlueCheeseSwitch){
            if(PlayerUtils.getItemInHotbarFromLore(true, "Blue Cheese") == -1){
                ...
                BetterMining.config.powBlueCheeseSwitch = false;
            }
        }
        ...
        if (BetterMining.config.powMineGemstone && PlayerUtils.getItemInHotbar(true, "Drill", "Gauntlet", "Pickonimbus") == -1) {
        ...
        if(BetterMining.config.powMineGemstone && !blocksAllowedToMine.contains(Blocks.stained_glass)){
            blocksAllowedToMine.add(Blocks.stained_glass_pane);
            blocksAllowedToMine.add(Blocks.stained_glass);
        } else if(!BetterMining.config.powMineGemstone && blocksAllowedToMine.contains(Blocks.stained_glass)){
    ...
    public void onTick(TickEvent.Phase phase){
        ...
        if(BetterMining.config.powAutosell){
            ...
            case UTurn: case NORMAL:
                ...
                if(frontShouldMineSlow()){
                    if(havePotentialObstacles()
                            ...
                            && BetterMining.config.powCenter
                            && BetterMining.config.powMineGemstone)
                        aote = true;
                }
                ...
                if(BetterMining.config.powPickaxeSwitch)
    ...
    private void updateState(){
        ...
        switch (currentState) {
            ...
            case NORMAL:
                ...
                break;
            case UTurn:
                if (MathUtils.getDistanceBetweenTwoBlock(BlockUtils.getPlayerLoc(), uTurnCachePos) > BetterMining.config.powLaneWidth || shouldTurn(3)) {
                    playerYaw = AngleUtils.get360RotationYaw(playerYaw + getRotAmount());
                    currentState = State.NORMAL;
                }
                break;
    ...
    public void onLastRender(RenderWorldLastEvent event) {
        ...
        if(rotation.rotating){
            ...
            this.rotation.updateInCircle(BetterMining.config.powRotateRadius, 3, playerYaw, BetterMining.config.powRotateRate);
        }
    }
