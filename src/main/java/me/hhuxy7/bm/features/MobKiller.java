...
import static me.hhuxy7.bm.handlers.KeybindHandler.leftClick;
import static me.hhuxy7.bm.handlers.KeybindHandler.rightClick;
...
public class MobKiller {
            ...
            case ATTACKING:
                ...
                if (BetterMining.config.useHyperionUnderPlayer) {
                    ...
                    if (AngleUtils.isDiffLowerThan(mc.thePlayer.rotationYaw, 89, 0.5f)) {
                        if (attackDelay.hasReached(BetterMining.config.mobKillerAttackDelay) && target.distance() < 6) {
                    ...
                    if (!BetterMining.config.customItemToKill.isEmpty()) {
                        weapon = PlayerUtils.getItemInHotbar(BetterMining.config.customItemToKill);
                    } else {
                    ...
                    if (!visible) {
                        ...
                        if (targeted) {
                            if (attackDelay.hasReached(BetterMining.config.mobKillerAttackDelay)) {
                                if (BetterMining.config.attackButton == 0) {
                ...
                break;
            case BLOCKED_VISION:
                if (NpcUtil.getEntityHp(target.entity) <= 0 || target.distance() > BetterMining.config.mobKillerScanRange) {
                    currentState = States.KILLED;
                    afterKillDelay.reset();
                    break;
                }
    ...
    public void onRenderWorldLastRotation(RenderWorldLastEvent event) {
        ...
        if (target != null && !BetterMining.config.useHyperionUnderPlayer) {
            int yawRotation;
            int pitchRotation;
            ...
            rotation.initAngleLock(yawRotation, pitchRotation, BetterMining.config.mobKillerCameraSpeed);
        } else if (target != null) {
            if (target.distance() > 5.5) return;

            rotation.initAngleLock(mc.thePlayer.rotationYaw, 89, BetterMining.config.mobKillerCameraSpeed);
        }
    }
