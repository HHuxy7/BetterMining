...
import me.hhuxy7.bm.utils.UngrabUtils;
...
import static me.hhuxy7.bm.handlers.MacroHandler.macros;
...
public class Failsafes {
    ...
    public void onBlockChange(BlockChangeEvent event) {
        ...
        if (blockChangeTimer.hasReached(1000)) {
            if (changesToBedrock > BetterMining.config.bedrockThreshold) {
    ...
    public void onWorldChange(WorldEvent.Load event) {
        ...
        if (mc.thePlayer == null || mc.theWorld == null)
            return;
        if (!BetterMining.config.disableOnWorldChange) return;
    ...
    public void onTick(TickEvent.ClientTickEvent event) {
        ...
        if (!BetterMining.config.playerFailsafe) return;
        ...
        if (bedrockCount > BetterMining.config.bedrockBackupThreshold) {
            bedrockFailsafeFake(false);
            return;
        }

        if (PlayerUtils.isNearPlayer(BetterMining.config.playerRad) && someoneIsCloseTimer == null){
            someoneIsCloseTimer = new Timer();
        } else if (!PlayerUtils.isNearPlayer(BetterMining.config.playerRad) && someoneIsCloseTimer != null) {
            someoneIsCloseTimer = null;
        }

        if (PlayerUtils.isNearPlayer(BetterMining.config.playerRad) && someoneIsCloseTimer != null && someoneIsCloseTimer.hasReached(BetterMining.config.playerDetectionThreshold)) {
            PingAlert.sendPingAlert();
            DisableMacros();
    ...
    public void onPacket(ReceivePacketEvent event) {
        if (!BetterMining.config.stopMacrosOnRotationCheck) return;
        if (macros.stream().noneMatch(Macro::isEnabled)) return;
        if (mc.thePlayer == null || mc.theWorld == null) return;
    ...
    public static TrayIcon createNotification(String text, SystemTray tray) {
        TrayIcon trayIcon = new TrayIcon(new BufferedImage(1, 1, BufferedImage.TYPE_3BYTE_BGR), "BetterMining Failsafe Notification");
        trayIcon.setToolTip("BetterMining Failsafe Notification");
        try {
            tray.add(trayIcon);
        ...
        trayIcon.displayMessage("BetterMining - Failsafes", text, TrayIcon.MessageType.WARNING);
        return trayIcon;
    }
    ...
    public static void bedrockFailsafeFake(boolean resumeMacro) {
        ...
        fakeMovementThread = new Thread(() -> {
            if(!BetterMining.config.fakeMovements)
                return;
    ...
    public static void fakeMovement(boolean resumeMacro) {
        ...
        TrayIcon trayIcon;
        if(BetterMining.config.notifications)
